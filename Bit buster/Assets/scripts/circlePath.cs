	...
	private bool checkForCombinations(bool chain= false)
	{
		for(int i= 0; i < objectList.Count; i++)
		{
			...
			if(itm.GetComponent<BitObject>().objectType != "bit")
			{
				...
				BitObject bit1= objectList[prev].GetComponent<BitObject>();
				BitObject bit2= objectList[next].GetComponent<BitObject>();

				if(bit1.objectType == bit2.objectType && bit1.value == bit2.value)
				{
